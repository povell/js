...
var scoreNode; // Узел для вывода результата

function init() {
    prepareGameField(); // Генерация поля

    var wrap = document.getElementsByClassName('wrap')[0];
    scoreNode = document.getElementsByClassName('score')[0];
...
/**
 * проверка на еду иди препятствие
 * @param unit
 * @returns {boolean}
 */
function haveFoodOrBarrier(unit) {
    var check = false;
    ...
    else if (unit_classes.includes('barrier-unit')) {
        check = true;
        finishTheGame();
    }
    return check;
